...
{
    public class LE_ETC : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the MyComponent2 class.
        /// </summary>
        public LE_ETC()
            : base("Lateral Efficiency", "LE",
        ...
        {
            pManager.AddNumberParameter("Lateral Efficiency", "LE", "Lateral Efficiency", GH_ParamAccess.list);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            Grasshopper.DataTree<double> LE_final = new Grasshopper.DataTree<double>();
            ...
            for (int s = 0; s < max; s++)
            {
                ...
                for (int r = 0; r < Rec[s].Rec_List.Length; r++)
                {
                    ...
                    for (int o = (int)Oct.T0; o <= Oct.T1; o++)
                    {
                        ...
                        double[] LETC = Pachyderm_Acoustic.Utilities.IR_Construction.ETCurve_1d(D[s], IS[s], Rec[s], Rec[s].CutOffTime, Rec[s].SampleRate, o, r, false, Alt, Azi, true)[1];
                        S[(int)(o - Oct.T0)] = ETC;
                        direct[(int)(o - Oct.T0)] = (int)Math.Round(D[s].Time(r) * Rec[s].SampleRate);
                        LE.Add(Pachyderm_Acoustic.Utilities.AcousticalMath.Lateral_Efficiency(ETC, LETC, Rec[s].SampleRate, (double)direct[(int)(o - Oct.T0)] / (double)Rec[s].SampleRate, false));
                    }

                    if (s == 0 && r == 0)
                    {
                        LE_final = new Grasshopper.DataTree<double>(LE, new Grasshopper.Kernel.Data.GH_Path(new int[2] { 0, 0 }));
                    }
                    else
                    {
                        int[] path = new int[2] { s, r };
                        LE_final.EnsurePath(path);
                        LE_final.AddRange(LE, new Grasshopper.Kernel.Data.GH_Path(path));
                    }
                }
            }
            DA.SetDataTree(0, LE_final);
        }
        ...
        {
            get { return new Guid("251477CD-DE9E-48E0-9F97-1B68240E1F09"); }
        }
    }
}